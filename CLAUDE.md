...
- ブランド名: 「整うデスク」。SNS（@totonou_desk）で先行運用中のブランドの新チャネル。
  詳細は `D:\Claude_Code\AI_DropShipping_Store\research\stage1-plan.md`
- コンセプト・トーン: 整理整頓・ミニマリズムを軸に、サウナの「ととのう」に通じる心地よさ・チル感を大切にする。簡潔で、煽らない。「絶対に」「一番」等の断定的な煽り文言は使わない
- 対象読者: 在宅ワーク・デスク環境の整え方に関心がある層

## 絶対規約

1. 記事本文の数量・順序は算用数字で書く（「一度に」「一箇所」のような慣用表現の漢数字は対象外。機械検査の範囲は `scripts/validate-article.ps1` を参照）
2. 絵文字は使用しない
